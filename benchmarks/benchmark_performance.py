...
"""
    ...
    def _generate_large_test_case(self, num_laws=100, events_per_law=10):
        ...
        code_lines = ["target large_project:", '    key:"Large Performance Test"', '    dictionnary:']
    ...
    def benchmark_lexer(self):
        ...
        for size, code in [("Small", self.small_code), ("Medium", self.medium_code), ("Large", self.large_code)]:
            ...
            results[size] = {
                ...
                'chars_per_second': len(code) / tokenization_time if tokenization_time > 0 else 0
            }
    ...
    def benchmark_parser(self):
        ...
        for size, code in [("Small", self.small_code), ("Medium", self.medium_code), ("Large", self.large_code)]:
            ...
            results[size] = {
                ...
                'ast_stats': ast_stats,
                'tokens_per_second': len(tokens) / parsing_time if parsing_time > 0 else 0
            }
            
            print(f"  Parse time: {parsing_time:.3f}s")
            print(f"  AST size: {ast_stats['total_laws']} laws, {ast_stats['total_targets']} targets")
            print(f"  Speed: {results[size]['tokens_per_second']:,.0f} tokens/s")
    ...
    def benchmark_zenith_analyser(self):
        ...
        for size, code in [("Small", self.small_code), ("Medium", self.medium_code), ("Large", self.large_code)]:
            ...
            avg_law_analysis = sum(law_analysis_times) / len(law_analysis_times) if law_analysis_times else 0
    ...
    def benchmark_memory_usage(self):
        ...
        for size, code in [("Small", self.small_code), ("Medium", self.medium_code)]:
            ...
            total_init = sum(stat.size_diff for stat in stats_init if stat.size_diff > 0)
            total_analysis = sum(stat.size_diff for stat in stats_analysis if stat.size_diff > 0)
    ...
    def benchmark_unparser(self):
        ...
        for size, code in [("Small", self.small_code), ("Medium", self.medium_code), ("Large", self.large_code)]:
            ...
            results[size] = {
                ...
                'lines_per_second': stats['total_lines'] / unparsing_time if unparsing_time > 0 else 0
            }
    ...
    def generate_summary(self):
        ...
        if 'lexer' in self.results:
            ...
            if lexer_large:
                print(f"  Lexer: {lexer_large.get('tokens_per_second', 0):,.0f} tokens/s")
        
        # Parser performance
        if 'parser' in self.results:
            parser_large = self.results['parser'].get('Large', {})
            if parser_large:
                print(f"  Parser: {parser_large.get('tokens_per_second', 0):,.0f} tokens/s")
        
        # Zenith Analyser performance
        if 'zenith_analyser' in self.results:
            za_large = self.results['zenith_analyser'].get('Large', {})
            if za_large:
                laws_per_second = 1 / za_large.get('avg_law_analysis_time_seconds', 1) if za_large.get('avg_law_analysis_time_seconds', 0) > 0 else 0
        ...
        if 'memory' in self.results:
            ...
            if memory_medium:
                print(f"  Memory usage: {memory_medium.get('total_memory_kb', 0):.1f} KB")
            ...
            if component in self.results:
                ...
                for size in sizes:
                    if size in self.results[component]:
                        time_val = self.results[component][size].get('tokenization_time_seconds') or \
                                  self.results[component][size].get('parsing_time_seconds') or \
                                  self.results[component][size].get('initialization_time_seconds')
                        if time_val:
                ...
                if len(times) >= 2:
                    ...
                    if small_time > 0:
                        ...
                        print(f"  {component.title()}: {scaling:.1f}x slower for {times[-1][0].lower()} input")
                        ...
                        for metric in ['tokens_per_second', 'lines_per_second']:
                            if metric in current_large and metric in previous_large:
                                ...
                                if previous_val > 0:
                                    ...
                                    comparisons.append((component, metric, current_val, previous_val, change))
            
            if comparisons:
                print("\n📊 Performance Changes:")
                for component, metric, current, previous, change in comparisons:
                    direction = "↑" if change > 0 else "↓"
                    print(f"  {component.title()}: {current:,.0f} {metric} ({direction}{abs(change):.1f}%)")
            else:
