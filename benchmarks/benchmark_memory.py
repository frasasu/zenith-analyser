...
class MemoryBenchmark:
    ...
    def benchmark_component_memory(self):
        ...
        for size_name, code in self.test_cases.items():
            ...
            size_results["zenith_analyser"] = {
                ...
                "target_count": (
                    len(analyser.target_analyser.targets) if analyser else 0
                ),
            }
            ...
            if analyser and analyser.law_analyser.get_law_names():
                ...
                size_results["analysis"] = {"memory_kb": analysis_memory / 1024}
            ...
            print(
                f"  ZenithAnalyser: {size_results['zenith_analyser']['memory_kb']:.1f} KB"
            )
    ...
    def benchmark_memory_scaling(self):
        ...
        for size_name, code in self.test_cases.items():
            ...
            results[size_name] = {
                ...
                "memory_per_char_bytes": (
                    total_memory / len(code) if len(code) > 0 else 0
                ),
            ...
            print(
                f"  Memory/char: {total_memory/len(code):.2f} bytes"
                if len(code) > 0
                else "  Memory/char: N/A"
            )
        ...
        for i in range(len(sizes) - 1):
            ...
            if small in results and large in results:
                size_ratio = (
                    results[large]["code_size_chars"]
                    / results[small]["code_size_chars"]
                )
                memory_ratio = (
                    results[large]["total_memory_kb"]
                    / results[small]["total_memory_kb"]
                )
        ...
        for data in scaling_data:
            ...
            print(
                f"    Size: {data['size_increase']:.1f}x, Memory: {data['memory_increase']:.1f}x"
            )
    ...
    def benchmark_memory_leaks(self):
        ...
        print(
            f"  Increase: {memory_increase:.1f} KB ({results['percent_increase']:.1f}%)"
        )
    ...
    def benchmark_peak_memory(self):
        ...
        for size_name in ["tiny", "small", "medium", "large"]:
            ...
            final_memory = sum(
                stat.size for stat in final_snapshot.statistics("filename")
            )
            ...
            print(
                f"  Peak/char: {peak_memory/len(code):.2f} bytes"
                if len(code) > 0
                else "  Peak/char: N/A"
            )
        ...
        for i in range(len(sizes) - 1):
            ...
            if small in results and large in results:
                ...
                memory_ratio = (
                    results[large]["peak_memory_kb"] / results[small]["peak_memory_kb"]
                )

                print(
                    f"  {small} → {large}: Size {size_ratio:.1f}x, Memory {memory_ratio:.1f}x"
                )
    ...
    def generate_summary(self):
        ...
        if "leak_detection" in self.results:
            ...
            print(
                f"  Leak detected: {'Yes' if leak_data.get('leak_detected') else 'No'}"
            )
