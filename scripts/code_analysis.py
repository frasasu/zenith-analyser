...
class CodeAnalyzer:
    ...
    def __init__(self, directory: str = "src"):
        ...
        self.results: Dict[str, Any] = {"files": {}, "summary": {}, "issues": []}
    ...
    def _calculate_summary(self) -> None:
        ...
        self.results["summary"] = {
            ...
            "lines_per_function": sum(f["lines"] for f in files)
            / max(sum(f["functions"] for f in files), 1),
            "comment_ratio": (
                sum(f["comments"] for f in files) / sum(f["lines"] for f in files)
            )
            * 100,
        }
    ...
    def print_report(self, output_format: str = "table") -> None:
        ...
        if self.results["issues"]:
            ...
            for issue in self.results["issues"]:
                print(
                    f"[{issue['severity'].upper()}] {issue['file']}: {issue['issue']}"
                )
    ...
    if covered_files:
        ...
        for file in covered_files:
            ...
            if analysis:
                ...
                coverage_pct = (
                    (executed / (total_lines - excluded)) * 100
                    if (total_lines - excluded) > 0
                    else 0
                )
...
def main():
    ...
    parser.add_argument(
        "--coverage", action="store_true", help="Include test coverage analysis"
    )
