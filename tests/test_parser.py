...
import pytest
import json
from src.zenith_analyser import Parser, ZenithParserError ,ZenithLexerError
...
def test_skip_whitespace(parser):
    ...
    parser.pos = 0
    while parser.pos < len(parser.tokens) and parser.tokens[parser.pos]['type'] != 'whitespace':
        parser.pos += 1
...
end_law
""", True),  # Should parse (semantic check elsewhere)
    ]
...
def test_parse_round_trip(sample_code):
    ...
    from src.zenith_analyser import Lexer, Parser, ASTUnparser
