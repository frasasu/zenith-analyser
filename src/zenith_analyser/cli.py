...
from . import ZenithAnalyser, ASTUnparser, Validator
...
def create_parser() -> argparse.ArgumentParser:
    ...
    analyze_parser.add_argument('--population', type=int, default=-1,
    ...
    unparse_parser.add_argument('--format', action='store_true',
                              help='Format output code')
    ...
    convert_parser.add_argument('--from', dest='from_format', 
                              choices=['zenith', 'json'], default='zenith',
                              help='Input format')
    convert_parser.add_argument('--to', choices=['zenith', 'json'], default='json',
                              help='Output format')
...
def convert_command(args: argparse.Namespace) -> None:
    ...
    if args.from_format == 'zenith' and args.to == 'json':
        ...
        print(f"Conversion from {args.from_format} to {args.to} not supported", 
              file=sys.stderr)
        sys.exit(1)
...
def format_text_output(result: dict) -> str:
    ...
    if 'name' in result:
        ...
        lines.append(f"Start: {result.get('start_datetime', {}).get('date', 'N/A')} "
        ...
        if 'simulation' in result:
            ...
            for event in result['simulation']:
                lines.append(f"  {event.get('event_name', 'N/A')}: "
        ...
        lines.append(f"  Total Duration: {stats.get('total_duration_minutes', 0)} minutes")
