...
from .utils import validate_identifier, validate_date, validate_time, validate_point
...
class Validator:
    ...
    def validate_code(self, code: str) -> List[str]:
        ...
        if len(code) > 1000000:  # 1MB limit
            self.warnings.append("Code is very large, consider splitting into smaller files")
    ...
    def _validate_basic_syntax(self, code: str):
        ...
        if invalid_chars:
            ...
            self.warnings.append(f"Potentially invalid characters found: {unique_chars}")
    ...
    def _validate_block_structure(self, code: str):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if leading_spaces % 4 != 0 and leading_spaces != 0:
                self.warnings.append(f"Line {i}: Indentation should be multiples of 4 spaces")
            ...
            if not in_law and not in_target:
                if ':' in stripped and not stripped.startswith(('law', 'target', '#')):
                    if not any(stripped.startswith(f"{kw}:") for kw in ZENITH_KEYWORDS):
                        self.errors.append(f"Line {i}: Statement outside law or target block")
            ...
            for word in words:
                if word in ZENITH_KEYWORDS:
                    ...
                    if not re.search(rf'\b{word}\s*[:\(]', line):
                        self.warnings.append(f"Line {i}: Reserved word '{word}' used as identifier")
    ...
    def _validate_token(self, token: Dict[str, Any], index: int):
        ...
        valid_types = [t for t in ZENITH_KEYWORDS] + [
            ...
            'string', 'identifier', 'newline', 'whitespace'
        ]
    ...
    def _validate_date_time(self, date_str: str, time_str: str):
        ...
        try:
            ...
            if dt.year < 2000 or dt.year > 2100:
                self.warnings.append(f"Date {date_str} is outside recommended range (2000-2100)")
        except ValueError:
    ...
    def _validate_period(self, period: str):
        ...
        try:
            ...
            if minutes > 5256000:  # 10 years in minutes
                self.warnings.append(f"Period {period} is very long ({minutes} minutes)")
        except:
            pass
    ...
    def _validate_dictionnary(self, dictionnary: List[Dict[str, Any]]):
        ...
        for i, entry in enumerate(dictionnary):
            ...
            if 'description' not in entry:
                self.errors.append(f"Dictionnary entry '{name}' missing 'description' field")
    
    def _validate_group(self, group: List[Dict[str, Any]], dictionnary: List[Dict[str, Any]]):
        ...
        for i, event in enumerate(group):
            ...
            if 'chronocoherence' in event:
                if not validate_point(event['chronocoherence']):
                    self.errors.append(f"Invalid chronocoherence in event {i}: '{event['chronocoherence']}'")
            
            if 'chronodispersal' in event:
                if not validate_point(event['chronodispersal']):
                    self.errors.append(f"Invalid chronodispersal in event {i}: '{event['chronodispersal']}'")
