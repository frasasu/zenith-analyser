...
def run_command(cmd, cwd=None, show_error=True):
    ...
    try:
        ...
        if result.returncode != 0 and show_error:
            if result.stderr:
                print(f"❌ Erreur: {result.stderr[:200]}")  # Afficher seulement les 200 premiers caractères
            else:
                print(f"❌ Code erreur: {result.returncode}")
            return False, result.stderr
        if result.stdout:
            print(f"✅ Sortie: {result.stdout[:100]}" if len(result.stdout) > 100 else f"✅ Sortie: {result.stdout}")
...
def push_to_github_force():
    ...
    if not success:
        if "failed to push some refs" in error or "non-fast-forward" in error:
            ...
            response = input("Voulez-vous forcer le push? (écrasera tout sur GitHub) (o/n): ")
            if response.lower() in ['o', 'oui', 'y', 'yes']:
