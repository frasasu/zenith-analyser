...
"""
    ...
    for law_name in laws.keys():
        ...
        start = datetime.strptime(
            f"{desc['start_date']} {desc['start_time']}", "%Y-%m-%d %H:%M"
        )
    ...
    for i, (name1, desc1) in enumerate(law_list):
        for name2, desc2 in law_list[i + 1 :]:
            start1 = datetime.strptime(
                f"{desc1['start_date']} {desc1['start_time']}", "%Y-%m-%d %H:%M"
            )
            ...
            start2 = datetime.strptime(
                f"{desc2['start_date']} {desc2['start_time']}", "%Y-%m-%d %H:%M"
            )
...
"""
    ...
    for event in all_events:
        ...
        resource = (
            event["event_name"].split("[")[1].split("]")[0]
        ...
        if len(events) > 1:
            ...
            events.sort(
                key=lambda e: datetime.strptime(
                    f"{e['start']['date']} {e['start']['time']}", "%Y-%m-%d %H:%M"
                )
            )
    ...
    for resource, events in resource_events.items():
        ...
        print(
            f"    Average assignment: {total_minutes/len(events):.0f} minutes"
            if events
            else "    No assignments"
        )
...
"""
    ...
    end_dates = [
        datetime.strptime(
            f"{p['end_datetime']['date']} {p['end_datetime']['time']}", "%Y-%m-%d %H:%M"
        )
        for p in phases.values()
    ]
...
"""
    ...
    total_value = 0
    activity_summary = {
        activity: {"hours": 0, "value": 0} for activity in value_rates.keys()
    }
    ...
    for law_name in analyser.law_analyser.get_law_names():
        ...
        for event in desc["simulation"]:
            ...
            if activity_type:
                ...
                print(
                    f"  {event_name.split('[')[0]}: {hours:.1f} hours = ${value:,.0f}"
                )
            else:
                print(
                    f"  {event_name}: {event['duration_minutes']/60:.1f} hours (unvalued)"
                )
    ...
    print(
        f"Average hourly rate: ${total_value/total_hours:,.0f}/hour"
        if total_hours > 0
    ...
    for activity, summary in activity_summary.items():
        if summary["hours"] > 0:
            percentage = (
                (summary["value"] / total_value) * 100 if total_value > 0 else 0
            )
            hourly_rate = (
                summary["value"] / summary["hours"] if summary["hours"] > 0 else 0
            )
            print(f"\n  {activity}:")
            print(
                f"    Hours: {summary['hours']:.1f} ({summary['hours']/total_hours*100:.1f}%)"
            )
    ...
    value_efficiency = (
        (total_value / potential_value) * 100 if potential_value > 0 else 0
    )
