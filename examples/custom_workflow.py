...
from zenith_analyser import ASTUnparser, ZenithAnalyser
...
""",
    ...
    results = {}
    summary = {"total_projects": len(zenith_files), "total_hours": 0, "projects": []}
    ...
    for filename, code in zenith_files.items():
        ...
        try:
            ...
            print(
                f"  ✓ {project_name}: {hours:.1f} hours, {analysis['event_count']} events"
            )
    ...
    print(
        f"Average hours per project: {summary['total_hours']/summary['total_projects']:.1f}"
    )
    ...
    for project in summary["projects"]:
        print(
            f"  {project['name']}: {project['start_date']}, {project['hours']:.1f} hours"
        )
...
"""
    ...
    return {"csv_data": csv_data, "ical_events": ical_events, "gantt_data": gantt_data}
        ...
        try:
            ...
            for i in range(len(law_data["simulation"]) - 1):
                ...
                end1 = datetime.strptime(
                    f"{event1['end']['date']} {event1['end']['time']}", "%Y-%m-%d %H:%M"
                )
            ...
            if len(law_data["simulation"]) > 6:
                ...
                print(
                    f"  💡 Law has {len(law_data['simulation'])} events (potentially fragmented)"
                )
            ...
            if off_hours_events:
                ...
                print(
                    f"  ⚠ Found {len(off_hours_events)} event(s) outside normal hours"
                )
...
"""
    ...
    try:
        ...
        calculated_duration = sum(
            e["duration_minutes"] for e in law_analysis["simulation"]
        )
        if abs(law_analysis["total_duration_minutes"] - calculated_duration) > 1:
            semantic_issues.append(
                f"Duration mismatch: {law_analysis['total_duration_minutes']} vs {calculated_duration}"
            )
    ...
    if avg_time < 0.1:  # Less than 100ms per analysis
        ...
        print(
            f"  ⚠ Performance test warning: {avg_time*1000:.1f}ms per analysis (slow)"
        )
...
"""
    ...
    executive_summary = {
        ...
        "project_summary": {
            ...
            "total_duration_hours": analysis["corpus_statistics"][
    ...
    print(
        f"  ✓ Executive summary generated with {len(executive_summary['key_findings'])} findings"
    )
    ...
    print(
        f"  ✓ Technical report generated with {len(technical_report['law_analyses'])} law analyses"
    )
    ...
    print(
        f"  ✓ Visualization data generated with {len(visualization_data['timeline_data'])} timeline entries"
    )
    ...
    report_formats = {
        ...
        "csv_snippet": lambda data: "\n".join(
            [",".join(row.keys()) for row in data.get("timeline_data", [][:3])]
        ),
    }
    ...
    for format_name, formatter in report_formats.items():
        try:
            ...
            exported_reports[format_name] = (
                exported[:500] + "..." if len(exported) > 500 else exported
            )
